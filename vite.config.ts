...
// Use an async IIFE so that vite is only imported dynamically, avoiding
// static resolution at module load time in production environments.
export default (async () => {
  const { defineConfig } = await import("vite");

  // Load all plugins dynamically to avoid needing them as production dependencies
  const [react, tailwindcss, { jsxLocPlugin }, { vitePluginManusRuntime }] = await Promise.all([
